...
if [ ! -f "$CONFIG_HOME/config/node_key.json" ]; then
	...
	if [ -n "${EVM_SIGNER_PASSPHRASE:-}" ]; then
		init_flags="$init_flags --chain_id=noble-applayer-devnet --rollkit.node.aggregator=true --rollkit.signer.passphrase $EVM_SIGNER_PASSPHRASE"
...
# Modify genesis time
sed -i 's/"start_time": *"[^"]*"/"start_time": "2025-09-24T10:00:00.000000000Z"/' ${CONFIG_HOME}/config/genesis.json
